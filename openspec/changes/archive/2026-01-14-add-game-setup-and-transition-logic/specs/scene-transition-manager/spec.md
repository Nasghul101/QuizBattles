# Specification: Scene Transition Manager

**Capability ID:** `scene-transition-manager`  
**Type:** System  
**Status:** Draft

## Purpose

Provides centralized, reusable scene transition functionality with visual effects (fade) for consistent user experience across all scene changes in the game.
...
```
Transitions to a new scene with optional parameters.
- `scene_path`: Path to the target scene (e.g., "res://scenes/ui/gameplay_screen.tscn")
- `params`: Dictionary of parameters to pass to the target scene's `initialize()` method
...
- Implemented as autoload singleton: `TransitionManager`
- Uses CanvasLayer with layer 100 to ensure overlay is on top
- ColorRect size must match viewport size
- Tween easing: `TRANS_SINE` with `EASE_IN_OUT` recommended
- Script file: `autoload/transition_manager.gd`
