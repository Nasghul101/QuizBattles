# Spec: Trivia Question Service

## Overview

A singleton autoload service that fetches trivia questions from the Open Trivia Database API, caches them in memory, and provides fallback to local questions when the API is unavailable. Consolidates Open Trivia DB categories and returns questions in their native format.
...
No external configuration needed. Category mappings are hardcoded in the service.

## Dependencies

- Godot 4.5+ HTTPRequest node
- JSON parsing (built-in)
- Fallback JSON file at `res://data/fallback_questions.json`

## Performance Considerations

- HTTP requests are async (non-blocking)
- JSON parsing is synchronous but lightweight (small question payloads)
- Cache lookup is O(1) dictionary access
- Memory footprint: ~50 questions × ~500 bytes ≈ 25KB (negligible)

## Testing Considerations

- Mock HTTPRequest responses for unit tests
- Test cache hit/miss scenarios
- Test fallback JSON loading
- Test category mapping logic
- Test error signal emissions
- Integration test with real API (manual/optional)

## Future Enhancements

- HTML entity decoding for question text
- Persistent disk cache for faster offline mode
- Smarter cache eviction (LRU instead of FIFO)
- Support for custom question sources (plugins)
