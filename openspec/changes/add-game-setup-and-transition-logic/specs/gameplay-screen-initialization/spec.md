# Specification: Gameplay Screen Initialization

**Capability ID:** `gameplay-screen-initialization`  
**Type:** Component  
**Status:** Draft

## Purpose

Receives and stores game configuration (rounds and questions) from the setup screen, maintaining state for future gameplay logic implementation and eventual Firebase persistence integration.
...
```
Sets the game configuration for this gameplay session.
- `rounds`: Number of rounds to play (categories to answer)
- `questions`: Number of questions per round
...
```
Stores the current game configuration.

---

## Dependencies

- `scene-transition-manager` - Called by transition manager during scene loading
- `setup-screen-logic` - Receives values from setup screen

---

## Technical Notes

- Uses GDScript static typing for all parameters and properties
- No validation logic required at this stage (add when game logic implemented)
- Script file: `scenes/ui/gameplay_screen.gd`
- Configuration values stored as simple integer properties
- Future work: Add validation (min/max bounds), Firebase sync, state persistence
